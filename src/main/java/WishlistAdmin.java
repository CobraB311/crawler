...
public class WishlistAdmin {
    ...
    private void loadConfigAndData() {
        ...
        try (InputStream in = getConfigStream()) {
            if (in == null) throw new IOException("config.properties niet gevonden.");
            props.load(in);
            ...
            String pathsRaw = props.getProperty("wishlist.localPaths", "");
            String[] pathsArr = pathsRaw.isEmpty() ? new String[0] : pathsRaw.split(",");
            
            Map<String, String> paths = new HashMap<>();
            for(int i = 0; i < urls.size(); i++) {
                if(i < pathsArr.length) paths.put(urls.get(i).trim(), pathsArr[i].trim());
            }
    ...
    private void refreshTable() {
        ...
        for (var item : controller.getAllItems()) {
            ...
            ui.mainModel.addRow(new Object[]{item.getString("id"), item.getString("naam"), p, "Wachtend..."});
        }
    }
        ...
        new Thread(() -> {
            ...
            for (int i = 0; i < controller.getAllItems().size(); i++) {
                ...
                controller.scanSingleItemParallel(i, ui.bolCb.isSelected(), ui.legoCb.isSelected(), 
                        ui.amazonCb.isSelected(), ui.dreamCb.isSelected(), finishedIdx -> {
                    SwingUtilities.invokeLater(() -> {
                        ui.mainModel.setValueAt("Klaar", finishedIdx, 3);
                        ui.progress.setValue(finishedIdx + 1);
                        if (ui.mainTable.getSelectedRow() == finishedIdx) updateDetails();
                    });
                });
    ...
    private void handleSave(boolean copy) {
        ...
        String sel = (String) JOptionPane.showInputDialog(ui, "Kies bestand:", "Save", 3, null, options, options[0]);
        if (sel != null) {
            ...
            if (copy) Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(formatted), null);
                ...
                try { controller.save(sel, formatted); JOptionPane.showMessageDialog(ui, "Opgeslagen!"); refreshTable(); }
    ...
    private void openUrl() {
        ...
        if (row != -1) {
            try { Desktop.getDesktop().browse(new URI(ui.detailTable.getValueAt(row, 3).toString())); } catch(Exception e){}
        }
    }

    public static void main(String[] args) { SwingUtilities.invokeLater(WishlistAdmin::new); }
}